...
const Work = () => {
  ...
  const handleFilter = (filterItem) => {
    ...
    if (filterItem.toLowerCase() === "all") {
      ...
      const data = works.filter(
        (item) => item.tags && item.tags.includes(filterItem.toLowerCase())
      );
      setFilterWorks(data);
    }
  };
  ...
  return (
    <div className="py-14 h-full flex flex-col justify-center items-center">
      ...
      <div className="flex gap-10">
        ...
        {filterWorks.map((work, index) => (
          <div key={`work-${index}`} className="flex flex-col justify-center items-center">
            <div
              key={`work-${index}`}
            ...
            {/* project name */}
            <h3 className="text-xl text-black-color font-semibold py-5">{work.title}</h3>
          </div>
        ))}
      </div>
    </div>
